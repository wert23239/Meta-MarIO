...
FilenameTraining = "level11_startNoEnemy.state"
...
--[[
TimeoutConstant: How long it take till the enemies to despawn.M
--]]
TimeoutConstant = 80
...
function LevelChange()
	if NetLevel~=marioLevel or NetWorld~=marioWorld then
		...
		if forms.ischecked(trainingMode) then
			training = true
		end
...
trainingMode = forms.checkbox(form, "Training Mode", 120, 180)
...
--Rightmost Label is the fitness for how far right you can go
TopmostLabel = forms.label(form, "Topmost ", 5, 255)
--Each pixel right move is multiplied by this number
TopmostAmount = forms.textbox(form, 1, 60, 20, nil, 120, 255)
--If an organism reaches farther than right than ever before during a generation reset the timeout constant
TopmostTimeout = forms.checkbox(form, "", 270, 255 , "true")
--Toggle the rightmost fitness type
TopmostFitness = forms.checkbox(form, "", 230, 255, "true")
...
while true do
	...
	if(inPlay()) then
		...
		if marioY > topmost and ( forms.ischecked(TopmostFitness) or forms.ischecked(TopmostTimeout)  )then
			topmost = marioY
			if forms.ischecked(TopmostTimeout) then
				timeout = tonumber(forms.gettext(TimeoutConstantText))
			end
		end
		...
		if timeout + timeoutBonus <= 0  or TimeoutAuto == true then
			...
			if forms.ischecked(TopmostFitness) then
				fitnesscheck[3] = tonumber(forms.gettext(TopmostAmount))*(topmost - NetX)
			end
		...
		if forms.ischecked(TopmostFitness) then
				fitnessDisplay[3] = (topmost - NetX)*tonumber(forms.gettext(TopmostAmount))
		end
		if forms.ischecked(NoveltyFitness) then
		...
		if not forms.ischecked(hideBanner) then
			...
			gui.drawText(0, 24, "RM: " .. math.floor(fitnessDisplay[0]), 0xFF000000, 8)
			gui.drawText(50, 24, "TM: " .. math.floor(fitnessDisplay[3]), 0xFF000000, 8)
			gui.drawText(100, 24, "NS: " .. math.floor(fitnessDisplay[1]), 0xFF000000, 8)
			gui.drawText(150, 24, "SO: " .. math.floor(fitnessDisplay[2]), 0xFF000000, 8)
		end
